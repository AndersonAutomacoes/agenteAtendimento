...
3. **Sem assinatura válida** (após cancelamento efetivo, expiração ou inadimplência fora da tolerância) — **sugestão adotada nesta spec:** **bloqueio total** ao uso operacional do produto para aquele tenant. Permanecem acessíveis apenas fluxos estritamente necessários para recuperação comercial e segurança: **landing page** (marketing / conversão pública, sem dados do tenant autenticado), **login/logout**, **tela de planos e Checkout Stripe**, **Customer Portal**, **mensagem de conta suspensa/inadimplência** e **webhooks Stripe** no backend. Não há modo “somente leitura” na v1; qualquer recurso que dependa de entitlement pago deve retornar **403/redirect** até existir novamente subscription em estado que confira direito de uso conforme seção 6.
